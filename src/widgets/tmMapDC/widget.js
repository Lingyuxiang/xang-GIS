	...
	return declare([BaseWidget], {
		...
		startup () {
			layui.use(['form','tree','laydate'], ()=>{
				...
				this._initExportFun();
				// this._openSZLayer();
				// this._testFeatureServer();
			})
		},
		_getAllRegion(){
			const _this = this;
			const url = window.serverUrl + 'dcController/queryDcPosition';

			serverUtils.excutePostRequest(url,"获取dc边界",{}).then((res)=>{
		...
		_testFeatureServer(){
			const _this = this;
			const fl = new FeatureLayer("https://devinmsarcgis01.com/server/rest/services/GDHK_SD/GDHK_SD_R_PIPELINE/FeatureServer/0", {
			...
			on(this.map, 'onZoomEnd', function() {
				window.maxOffset = calcOffset();
				fl.setMaxAllowableOffset(window.maxOffset);
			});

			function calcOffset() {
				return (_this.map.extent.getWidth() / _this.map.width);
			}

		},
		_initIndexOption(){
			const _this = this;
			// const url = window.serverUrl + 'commonController/queryLossDictionary?zoneType=DC';
			// serverUtils.excuteAjaxRequest(url,"获取指标",{
			// 	success (resp) {
			// 		if (resp.code == 0) {
			// 		  console.log(resp.data)
		...
		_bindEvent(){
			...
			this._form.on('submit(subm)', (data)=>{
				...
				const url = window.serverUrl + 'dcController/queryDcThematicValue';

				serverUtils.excutePostRequest(url,"获取dc指标",param,true).then((result)=>{
					this._createGraLayer();
					if(result.length == 0){
						layer.msg('未查询到指标', {
							icon: 2
					...
					let numResult = []
					for(let i of result){
						dealResult.push({itemCode: param.indicator,name:i.dcNo})
		...
		_createGraphic(data){
			...
			data.forEach((item,index)=>{
				dataObj[item.szNo]&&dataObj[item.dcNo].push(JSON.parse(item.position));
				if(dataObj[item.dcNo] == undefined){
					dataObj[item.dcNo] = [];
					console.log(index);
					dataObj[item.dcNo].push(JSON.parse(item.position));
				}
			})
			console.log(dataObj);
			let currentGra = [];
